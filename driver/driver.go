...
func GoListDriver(ctx context.Context, cfg *Config) (*DriverResponse, error) {
	cfg.context = ctx
...
func runNamedQueries(cfg *Config, driver driver, addPkg func(*Package), queries []string) ([]string, error) {
	...
	// startWalk := time.Now()
	gopathwalk.Walk(roots, add, gopathwalk.Options{ModulesEnabled: modRoot != "", Debug: false})
	// if debug {
...
func golistDriverCurrent(cfg *Config, words ...string) (*DriverResponse, error) {
	...
	// buf, err := getResp(ls, cfg, golistargs(cfg, words)...)
	// if err != nil {
	// 	return nil, err
	// }
